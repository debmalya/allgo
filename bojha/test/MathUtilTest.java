...
public class MathUtilTest {
	...
	public void getConbinatoricFactor2() {
		...
		Assert.assertEquals(747697931370219115L,
				MathUtil.getBinaryCoefficient(1000, 500));
	...
	public void testPrimeList() {
		...
		primeList.add(2);
		MathUtil.primeList(3, primeList);
		Assert.assertEquals(2, primeList.size());
		Assert.assertEquals(3, (int) primeList.get(primeList.size() - 1));

		MathUtil.primeList(4, primeList);
		Assert.assertEquals(2, primeList.size());
		Assert.assertEquals(3, (int) primeList.get(primeList.size() - 1));

		MathUtil.primeList(5, primeList);
		Assert.assertEquals(3, primeList.size());
		Assert.assertEquals(5, (int) primeList.get(primeList.size() - 1));

		MathUtil.primeList(6, primeList);
		Assert.assertEquals(3, primeList.size());
		Assert.assertEquals(5, (int) primeList.get(primeList.size() - 1));

		MathUtil.primeList(7, primeList);
		Assert.assertEquals(4, primeList.size());
		Assert.assertEquals(7, (int) primeList.get(primeList.size() - 1));

	}

	@Test
	public void testPrimeListInLoop() {
		List<Integer> primeList = new ArrayList<Integer>(100000);
		primeList.add(2);
		long start = System.currentTimeMillis();
		for (int i = 3; i < Integer.MAX_VALUE; i++) {
			MathUtil.primeList(i, primeList);
		}
		System.out.println("Time taken :" + (double)(System.currentTimeMillis() - start)/60000 + " min(s).");
