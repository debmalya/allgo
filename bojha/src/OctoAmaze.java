...
public class OctoAmaze {
	 ...
	 * 
	 * Can you write a function that takes a list of Jobs and puts them in an
	 * order that does not violate any dependencies. A -> B, B -> D, C -> [], D
	 * -> [], E -> [] A valid schedule might be, EDCBA, or DBCEA, but ABCDE is
	 * not valid.
	 */
	...
	public List<Job> schedule(List<Job> jobList) {
		...
		// O(Number of dependent jobs ) // can be quadratic (Worst case)
		while (!jobDependency.isEmpty()) {
			// check each job whether their dependent job already in the list or
			// not.
			// if dependent jobs are scheduled. Then only add that job and
			// remove that job
			// from jobDependency.
	...
	/**
	 * You will be given a list of numbers, where numbers may be repeated. Your
	 * method will return list of numbers which has even number of occurences
	 * 
	 * @param inputList
	 *            list containing numbers, numbers can be repeated.
