...
public class Company {
	 ...
	 * 
	 * Time complexity : Best Case : 1 (Directly reporting) 
	 * Worst Case : O(V**2) V is the cardinality of the vertices.
	 * Breadth first search is used here.
	 * breadth of the tree.
	 * 
	 * Space complexity : This is tree type structure. CEO is the root. All
	 * others are like branches. Not introduced any new data structure. Utilized existing data. No additional space.
	 * 
	 ...
	 * @param ceo - CEO of the company.
	 * @param firstEmployee - First employee
	 * @param secondEmployee - Second employee
	 * @return - Nearest common boss of the employees.
	 */
	private static Employee getCommonBoss(Employee ceo, Employee firstEmployee,
			Employee secondEmployee) {
		if (getBoss(ceo, firstEmployee).getId() == secondEmployee.getId()) {
			return secondEmployee;
		}
		if (getBoss(ceo, secondEmployee).getId() == firstEmployee.getId()) {
			return firstEmployee;
		}
		Employee firstEmployeeBoss = getBoss(ceo, firstEmployee);
		Employee secondEmployeeBoss = getBoss(ceo, secondEmployee);

		while (firstEmployeeBoss != null && secondEmployeeBoss != null
				&& firstEmployeeBoss.getId() != secondEmployeeBoss.getId()) {
	 ...
	 * @param ceo - CEO of the company
	 * @param employee - any employee of the company.
